package pc;

import java.awt.BorderLayout;
import java.awt.GridLayout;
import java.awt.TextArea;
import java.awt.TextField;
import java.awt.event.*;
import javax.swing.JButton;
import javax.swing.JToggleButton;
import javax.swing.JFrame;
import javax.swing.JPanel;
import javax.swing.JLabel;
import java.io.*;

/**
 * Uploads datalog from the NXT <br>
 * A Java application <br>
 * Press the button to set row size; The data can be copied and pasted into a
 * spread sheet for analysis & graphing <br>
 * status field shows messages
 * 
 * @author Roger Glassey revised 05.11.2007
 * 
 * 
 */
public class DownloadDL extends JFrame implements ActionListener {

	private JButton startButton = new JButton("Start Download");
	private TextField statusField = new TextField(20);
	private TextField lengthField = new TextField(5);
	private TextField nameField = new TextField(12);
	private int _recordCount = 0;
	private int _rowLength = 4;
	private String _nxt = "NXT";
	private boolean _useBT = true;
	private boolean connected = false;
	private DataInputStream dataIn = null;
	private OutputStream os = null;
	private BluetoothConnector con;

	/**
	 * screen area to hold the downloaded data
	 */
	private TextArea theLog;

	/**
	 * Constructor builds GUI
	 */
	public DownloadDL() {
		setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
		setTitle(" Download Datalogger");
		setSize(500, 600);
		JPanel p1 = new JPanel(); // holds button and text field
		p1.add(new JLabel("Row Length:"));
		p1.add(lengthField);
		lengthField.setText("4");
		p1.add(new JLabel("name or address"));
		p1.add(nameField);
		p1.add(startButton);
		startButton.addActionListener(this);
		JPanel p2 = new JPanel();// holds label and text field
		p2.add(new JLabel("  Status:"));
		p2.add(statusField);

		JPanel panel = new JPanel(); // North area of the frame
		panel.setLayout(new GridLayout(2, 1));
		panel.add(p1);
		panel.add(p2);
		;
		add(panel, BorderLayout.NORTH);
		theLog = new TextArea(40, 40); // Center area of the frame
		getContentPane().add(theLog, BorderLayout.CENTER);
		statusField.setText("using BlueTooth");

	}

	/**
	 * Required by action listener; only action is generated by the get Length
	 * button
	 */
	public void actionPerformed(ActionEvent e) {
		// if(e.getSource()== startButton)
		// {
		_rowLength = Integer.parseInt(lengthField.getText());
		_nxt = nameField.getText();
		System.out.println(_rowLength + "  " + _nxt);
		_recordCount = 0;
		startDownload();
		// }
		// if(e.getSource()==BTButton)
		// {
		// _useBT = !_useBT;
		// if(_useBT){
		// setMessage("using BlueTooth");
		// }
		// else setMessage("using USB");
		// }
	}

	/**
	 * Initialize the display Frame <br>
	 */
	public static void main(String[] args) {
		DownloadDL frame = new DownloadDL();
		frame.setVisible(true);

	}

	private void startDownload() {
		float length = 0;
		_nxt = nameField.getText();
		setMessage("startDL");
		System.out.println("startDL");
		con = new BluetoothConnector();
		con.startConnector(_nxt);
		dataIn = con.getDataIn();
		os = con.getOutputStream();
		connected = true;
		int b = 15;
		try {
			os.write(b);
			os.flush();
		} catch (IOException e) {
			System.out.println(e + " write ");
		}
		setMessage("write " + b);
		setMessage("reading");
		try {
			length = dataIn.readFloat();
		} catch (IOException e) {
			System.out.println(e + " read dataIn ");
		}
		setMessage("length " + length);
		float x = 0;
		try {
			for (int i = 0; i < length; i++) {
				if (0 == _recordCount % _rowLength)
					theLog.append("\n");
				x = dataIn.readFloat();
				theLog.append(x + "\t ");
				_recordCount++;
				System.out.println("" + _recordCount + " " + x);
			}
			dataIn.close();
		} catch (IOException e) {
			System.out.println(e);
		}

	}

	/**
	 * messages generated by PCcomSerial show in the status Field
	 */
	public void setMessage(String s) {
		statusField.setText(s);
	}
}
